...
- 节点下拉菜单更新的逻辑已经随着ComfyUI版本的更新而逐渐失效，之前的逻辑是无论是节点丶分类还是设置翻译文件中，只要有对应键值出现，下拉选项都会被进行翻译，这导致了很多问题。

- 日后插件节点中的所有下拉选项翻译，将只在设置菜单的翻译文件中进行，这会让翻译文件的结构更加清晰，方便了后期维护和管理的同时，也减少了贡献者的翻译负担，之后的贡献者提交的节点翻译文件中若包含下拉选项翻译，只需提交到设置菜单的翻译文件中即可。

- 这也意味着很多翻译可以更合理的复用，假设一个插件中同时具有【节点右键菜单丶设置面板菜单丶下拉选项菜单丶独立面板菜单】中的任意一处只要出现相同键值，就无需重复翻译，只需进行一次翻译即可。

- 【请提交独立节点名称的json文件，太过混淆的文件结构不利于项目日后的维护】
