...
- 节点下拉菜单更新的逻辑已经随着ComfyUI版本的更新而逐渐失效，现在插件节点中的所有下拉选项翻译将属于设置菜单的一部分，我会陆续迁移已有的下拉翻译。

- 为了方便项目日后的维护，也为了减少贡献者的翻译负担，之后的贡献者提交的节点翻译文件中若包含下拉选项翻译，只需提交到设置菜单的翻译文件中即可，节点本身的下拉选项翻译将不再需要提交，这也意味着当一个插件中的多个节点拥有相同下拉选项时，无需重复翻译，只需在设置菜单中翻译一次即可。【请提交独立节点名称的json文件，太过混淆的文件结构不利于项目日后的维护】
